...
public class PatchsetOptionsTest {
  ...
  private static final PatchsetSubject WHERE_IS_MY_MIND_SUBJECT =
      PatchsetSubject.of(WHERE_IS_MY_MIND);
  ...
  public void WHEN_default_THEN_push_target_has_no_option() {
    String pushTarget =
        PatchsetOptions.DEFAULT.buildGitPushTarget(REVISION_ID, BranchShortName.MASTER);
  ...
  public void WHEN_option_subject_WHERE_IS_MY_MIND_THEN_it_suffix_push_with_WHERE_IS_MY_MIND() {
    PatchsetOptions patchOptions =
        PatchsetOptions.builder().subject(WHERE_IS_MY_MIND_SUBJECT).build();
