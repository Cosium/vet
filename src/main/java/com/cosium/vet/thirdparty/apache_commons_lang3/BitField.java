 ...
 *
 * <p>Each {@code BitField} is constructed with a mask value, which indicates the bits that will be
 * used to store and retrieve the data for that field. For instance, the mask {@code 0xFF} indicates
 * the least-significant byte should be used to store the data.
 *
 * <p>As an example, consider a car painting machine that accepts paint instructions as integers.
 * Bit fields can be used to encode this:
 *
 * <pre>
 ...
 * </pre>
 *
 * <p>Using these {@code BitField} instances, a paint instruction can be encoded into an integer:
 *
 * <pre>
...
public class BitField {
   ...
   *
   * @param mask the mask specifying which bits apply to this BitField. Bits that are set in this
   *     mask are the bits that this BitField operates on
   */
   ...
   *
   * <p>Many users of a BitField will want to treat the specified bits as an int value, and will not
   * want to be aware that the value is stored as a BitField (and so shifted left so many bits).
   *
   ...
   *
   * <p>This is most commonly used for a single-bit field, which is often used to represent a
   * boolean value; the results of using it for a multi-bit field is to determine whether *any* of
   * its bits are set.
   *
   * @param holder the int data containing the bits we're interested in
   ...
   *
   * <p>This is a stricter test than {@link #isSet(int)}, in that all of the bits in a multi-bit set
   * must be set for this method to return {@code true}.
   *
   * @param holder the int data containing the bits we're interested in
