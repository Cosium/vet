...
import com.cosium.vet.thirdparty.apache_commons_lang3.reflect.MethodUtils;
...
public class EventUtils {
  ...
  /**
   * Adds an event listener to the specified source. This looks for an "add" method corresponding to
   * the event type (addActionListener, for example).
   *
   ...
   */
  public static <L> void addEventListener(
      final Object eventSource, final Class<L> listenerType, final L listener) {
    try {
   ...
   * @param listenerType the listener interface (ActionListener.class, SelectionListener.class,
   *     etc.)
   * @param eventTypes the event types (method names) from the listener interface (if none
   *     specified, all will be supported)
   */
  public static <L> void bindEventsToMethod(
      final Object target,
  ...
  private static class EventBindingInvocationHandler implements InvocationHandler {
    ...
    EventBindingInvocationHandler(
        final Object target, final String methodName, final String[] eventTypes) {
      this.target = target;
     ...
     */
    @Override
    public Object invoke(final Object proxy, final Method method, final Object[] parameters)
        throws Throwable {
    ...
    private boolean hasMatchingParametersMethod(final Method method) {
      return MethodUtils.getAccessibleMethod(
              target.getClass(), methodName, method.getParameterTypes())
          != null;
    }
  }
}
