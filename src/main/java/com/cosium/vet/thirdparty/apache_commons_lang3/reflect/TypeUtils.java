 ...
 */
package com.cosium.vet.thirdparty.apache_commons_lang3.reflect;

import com.cosium.vet.thirdparty.apache_commons_lang3.ArrayUtils;
import com.cosium.vet.thirdparty.apache_commons_lang3.ClassUtils;
...
import com.cosium.vet.thirdparty.apache_commons_lang3.builder.Builder;
...
public class TypeUtils {
  ...
  /**
   * {@code TypeUtils} instances should NOT be constructed in standard programming. Instead, the
  ...
  /**
   * Look up {@code var} in {@code typeVarAssigns} <em>transitively</em>, i.e. keep looking until
   * the value found is <em>not</em> a type variable.
   *
  ...
  private static Type unrollVariableAssignments(
      TypeVariable<?> var, final Map<TypeVariable<?>, Type> typeVarAssigns) {
    Type result;
    do {
   ...
   * arguments such as {@code Outer<K,V>.Inner<T>.DeepInner<E>} . The arguments are returned in a
   ...
   * java.util.Hashtable Hashtable} are also both {@code Object}. In cases where the interface
   * specified by {@code toClass} is (indirectly) implemented more than once (e.g. where {@code
   * toClass} specifies the interface {@link Iterable Iterable} and {@code type} specifies a
   * parameterized type that implements both {@link Set Set} and {@link java.util.Collection
   * Collection}), this method will look at the inheritance hierarchy of only one of the
   * implementations/subclasses; the first interface encountered that isn't a subinterface to one of
   * the others in the {@code type} to {@code toClass} hierarchy.
   *
   ...
   * Iterable}. Since {@code TreeSet}'s parameter maps (indirectly) to {@code Iterable}'s parameter,
   * it will be able to determine that based on the super type {@code Iterable<? extends
   * Map<Integer, ? extends Collection<?>>>}, the parameter of {@code TreeSet} is {@code ? extends
   * Map<Integer, ? extends Collection<?>>}.
   *
   * @param cls the class whose type parameters are to be determined, not {@code null}
   * @param superType the super type from which {@code cls}'s type arguments are to be determined,
   *     not {@code null}
   * @return a {@code Map} of the type assignments that could be determined for the type variables
   ...
   *
   * <p>since {@code List} is a subinterface of {@code Collection}, this method will return the
  ...
  /**
   * Returns an array containing the sole value of {@link Object} if {@link
  ...
  /**
   * Returns an array containing a single value of {@code null} if {@link
  ...
  /**
   * Get the raw type of a Java type, given its context. Primarily for use with {@link
  ...
  /**
   * Learn, recursively, whether any of the type parameters associated with {@code type} are bound
   * to variables.
   *
  ...
  public static final ParameterizedType parameterize(
      final Class<?> raw, final Type... typeArguments) {
   ...
   */
  public static final ParameterizedType parameterize(
      final Class<?> raw, final Map<TypeVariable<?>, Type> typeArgMappings) {
    ...
    return parameterizeWithOwner(
        null, raw, extractTypeArgumentsFrom(typeArgMappings, raw.getTypeParameters()));
  }
  ...
  public static final ParameterizedType parameterizeWithOwner(
    ...
    if (raw.getEnclosingClass() == null) {
      ...
      Validate.isTrue(
          TypeUtils.isAssignable(owner, raw.getEnclosingClass()),
   ...
   */
  public static final ParameterizedType parameterizeWithOwner(
      final Type owner, final Class<?> raw, final Map<TypeVariable<?>, Type> typeArgMappings) {
    Validate.notNull(raw, "raw class is null");
    Validate.notNull(typeArgMappings, "typeArgMappings is null");
    return parameterizeWithOwner(
        owner, raw, extractTypeArgumentsFrom(typeArgMappings, raw.getTypeParameters()));
  }
  ...
  private static boolean equals(final ParameterizedType p, final Type t) {
    if (t instanceof ParameterizedType) {
      ...
      if (equals(p.getRawType(), other.getRawType())
          && equals(p.getOwnerType(), other.getOwnerType())) {
  ...
  private static void appendRecursiveTypes(
      final StringBuilder buf, final int[] recursiveTypeIndexes, final Type[] argumentTypes) {
  ...
  private static int[] findRecursiveTypes(final ParameterizedType p) {
    final Type[] filteredArgumentTypes =
        Arrays.copyOf(p.getActualTypeArguments(), p.getActualTypeArguments().length);
    int[] indexesToRemove = new int[] {};
    for (int i = 0; i < filteredArgumentTypes.length; i++) {
      if (filteredArgumentTypes[i] instanceof TypeVariable<?>) {
        if (containsVariableTypeSameParametrizedTypeBound(
            ((TypeVariable<?>) filteredArgumentTypes[i]), p)) {
  ...
  private static boolean containsVariableTypeSameParametrizedTypeBound(
      final TypeVariable<?> typeVariable, final ParameterizedType p) {
  ...
  private static <T> StringBuilder appendAllTo(
      final StringBuilder buf, final String sep, final T... types) {
