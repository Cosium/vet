...
public class ThreadUtils {
  ...
  /**
   * Return the active thread with the specified id if it belongs to a thread group with the
   ...
   * @throws IllegalArgumentException if the specified id is zero or negative or the group name is
   *     null
   * @throws SecurityException if the current thread cannot access the system thread group
   * @throws SecurityException if the current thread cannot modify thread groups from this thread's
   *     thread group up to the system thread group
  ...
  /**
   * Return active threads with the specified name if they belong to a thread group with the
   * specified group name.
   *
   * @param threadName The thread name
   * @param threadGroupName The thread group name
  ...
  public static Collection<Thread> findThreadsByName(
    ...
    final Collection<ThreadGroup> threadGroups =
        findThreadGroups(new NamePredicate(threadGroupName));
  ...
  /**
   * Return all active thread groups excluding the system thread group (A thread group is active if
  ...
  public static Collection<Thread> findThreads(
      final ThreadGroup group, final boolean recurse, final ThreadPredicate predicate) {
  ...
  /**
   * Select all active threadgroups which match the given predicate and which is a subgroup of the
   * given thread group (or one of its subgroups).
   *
   * @param group the thread group
   * @param recurse if {@code true} then evaluate the predicate recursively on all threadgroups in
   *     all subgroups of the given group
   * @param predicate the predicate
   * @return An unmodifiable {@code Collection} of active threadgroups which match the given
  ...
  public static Collection<ThreadGroup> findThreadGroups(
      final ThreadGroup group, final boolean recurse, final ThreadGroupPredicate predicate) {
    Validate.isTrue(group != null, "The group must not be null");
    Validate.isTrue(predicate != null, "The predicate must not be null");
