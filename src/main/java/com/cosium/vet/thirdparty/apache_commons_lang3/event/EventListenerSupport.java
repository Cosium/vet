...
package com.cosium.vet.thirdparty.apache_commons_lang3.event;
...
/**
 * An EventListenerSupport object can be used to manage a list of event listeners of a particular
 * type. The class provides {@link #addListener(Object)} and {@link #removeListener(Object)} methods
 * for registering listeners, as well as a {@link #fire()} method for firing events to the
 * listeners.
 *
 * <p>To use this class, suppose you want to support ActionEvents. You would do:
 *
 * <pre><code>
 ...
 * </code></pre>
 *
 * <p>Serializing an {@link EventListenerSupport} instance will result in any non-{@link
 * Serializable} listeners being silently dropped.
 *
...
public class EventListenerSupport<L> implements Serializable {
  ...
  /**
   * The list used to hold the registered listeners. This list is intentionally a thread-safe
   * copy-on-write-array so that traversals over the list of listeners will be atomic.
   */
  private List<L> listeners = new CopyOnWriteArrayList<>();

  /**
   * The proxy representing the collection of listeners. Calls to this proxy object will sent to all
   ...
   *
   * @param listenerInterface the type of listener interface that will receive events posted using
   *     this class.
  ...
  /**
   * Creates an EventListenerSupport object which supports the provided listener interface using the
   * specified class loader to create the JDK dynamic proxy.
   *
   * @param listenerInterface the listener interface.
   * @param classLoader the class loader.
   * @throws NullPointerException if <code>listenerInterface</code> or <code>classLoader</code> is
   *     <code>null</code>.
   * @throws IllegalArgumentException if <code>listenerInterface</code> is not an interface.
   */
  ...
  /**
   * Returns a proxy object which can be used to call listener methods on all of the registered
   * event listeners. All calls made to this proxy will be forwarded to all registered listeners.
   *
   * @return a proxy object which can be used to call listener methods on all of the registered
   *     event listeners
   */
  public L fire() {
    return proxy;
  ...
  /**
   * Registers an event listener. Will not add a pre-existing listener object to the list if <code>
   * allowDuplicate</code> is false.
   *
   * @param listener the event listener (may not be <code>null</code>).
   * @param allowDuplicate the flag for determining if duplicate listener objects are allowed to be
   *     registered.
   * @throws NullPointerException if <code>listener</code> is <code>null</code>.
   * @since 3.5
   */
  ...
  /**
   * Get an array containing the currently registered listeners. Modification to this array's
   * elements will have no effect on the {@link EventListenerSupport} instance.
   *
   * @return L[]
   */
  ...
  private void initializeTransientFields(
      final Class<L> listenerInterface, final ClassLoader classLoader) {
    @SuppressWarnings("unchecked") // Will throw CCE here if not correct
  ...
  private void createProxy(final Class<L> listenerInterface, final ClassLoader classLoader) {
    proxy =
        listenerInterface.cast(
            Proxy.newProxyInstance(
                classLoader, new Class[] {listenerInterface}, createInvocationHandler()));
  }

  /**
   * Create the {@link InvocationHandler} responsible for broadcasting calls to the managed
   * listeners. Subclasses can override to provide custom behavior.
   *
  ...
  protected class ProxyInvocationHandler implements InvocationHandler {
     ...
     *
     * @param unusedProxy the proxy object representing a listener on which the invocation was
     *     called; not used
