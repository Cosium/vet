 ...
 *
 * <p>A {@code ThreadFactory} is used for instance by an {@code ExecutorService} to create the
 * threads it uses for executing tasks. In many cases users do not have to care about a {@code
 * ThreadFactory} because the default one used by an {@code ExecutorService} will do. However, if
 * there are special requirements for the threads, a custom {@code ThreadFactory} has to be created.
 *
 ...
 * </ul>
 *
 * <p>{@code BasicThreadFactory} wraps another thread factory which actually creates new threads.
 ...
 *
 * <p>Instances of {@code BasicThreadFactory} are not created directly, but the nested {@code
 * Builder} class is used for this purpose. Using the builder only the configuration options an
 * application is interested in need to be set. The following example shows how a {@code
 * BasicThreadFactory} is created and installed in an {@code ExecutorService}:
 *
 * <pre>
...
public class BasicThreadFactory implements ThreadFactory {
   ...
   *
   * <p>Using this builder class instances of {@code BasicThreadFactory} can be created and
  ...
  public static class Builder
      implements com.cosium.vet.thirdparty.apache_commons_lang3.builder.Builder<
          BasicThreadFactory> {
     ...
     *
     * @param factory the wrapped {@code ThreadFactory} (must not be <b>null</b>)
     * @return a reference to this {@code Builder}
     * @throws NullPointerException if the passed in {@code ThreadFactory} is <b>null</b>
     */
    ...
    /**
     * Sets the uncaught exception handler for the threads created by the new {@code
     * BasicThreadFactory}.
     *
     * @param handler the {@code UncaughtExceptionHandler} (must not be <b>null</b>)
     * @return a reference to this {@code Builder}
